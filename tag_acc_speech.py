...
if __name__ == '__main__':
    ...
    dic = make_dic()
    labels, tweets, speechs = load_labels_tweets()
    f = open("tag-%s-%s-speech-response.txt" % (sys.argv[1], sys.argv[2]), "w")
    pred_tags = list()
    for tweet, label, speech in tqdm(zip(tweets, labels, speechs)):
